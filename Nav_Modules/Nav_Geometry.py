...
class Edge:
    ...
    def Check_Edge_Circle_Collision(self, circle):
        """Checks if the edge collides with a given obstacle"""
        p1 = self.xy1; p2 = self.xy2; o = circle.center
        o_p1 = self.Vector_Diff(p1, o)
        o_p2 = self.Vector_Diff(p2, o)
        p2_p1 = self.Vector_Diff(p2, p1)
        p1_p2 = self.Vector_Diff(p1, p2)
        ...
        if min_dist <= circle.radius and max_dist >= circle.radius:
            return True # collision
        else:
            return False # no collision
...
class Circle:
    def __init__(self, radius, center, color):
        ...
        self.center = center
        self.color = color 
        self.patch = None  # Initialize patch to None

    def plot_circle(self, ax):
        """Plots the circle object"""
        circle = plt.Circle((self.center[0], self.center[1]), self.radius, color=self.color)
        self.patch = ax.add_patch(circle)
        return
    
    def remove_circle(self, ax):
        """Removes the circle object from the plot"""
        if self.patch is not None:  # Check if patch exists
            self.patch.remove()  # Remove patch from axes
            self.patch = None  # Set patch to None
        else:
            print("Circle not found in plot")
        return
...
class ObstacleSq(Rectangle):
    def __init__(self, origin, width, height):
        super().__init__(origin, width, height, color='brown')
